                ...
                public function appointments_list()
{
    ...
    try {
        ...
        if (!$this->db->table_exists(db_prefix() . 'appointments')) {
        ...
        // Get appointments using model method (much simpler)
        $appointments = $this->Appointments_model->get_appointments($filters);
        ...
        foreach ($appointments as $appointment) {
            $formatted_data[] = [
                ...
                'formatted_time' => date('h:i A', strtotime($appointment['start_time'])),
                'client_name' => $appointment['client_name'] ?? 'Unknown Client',
                'service_name' => $appointment['service_name'] ?? 'Unknown Service',
                'staff_full_name' => trim(($appointment['staff_firstname'] ?? '') . ' ' . ($appointment['staff_lastname'] ?? '')),
                'status' => $appointment['status'] ?? 'pending',
                'total_amount' => number_format((float)($appointment['total_amount'] ?? 0), 2),
                'payment_status' => $appointment['payment_status'] ?? 'unpaid',
                'duration_minutes' => $appointment['duration_minutes'] ?? 60,
                'consultation_id' => $appointment['consultation_id'] ?? null,
                'invoice_id' => $appointment['invoice_id'] ?? null
            ];
        }

        // Return simple response (like consultations)
        header('Content-Type: application/json');
        echo json_encode([
            'draw' => intval($this->input->get('draw')),
        ...
        log_message('error', 'Appointments list error: ' . $e->getMessage());
        ...
        echo json_encode([
            ...
            'error' => $e->getMessage(),
            'debug' => [
                'table_exists' => $this->db->table_exists(db_prefix() . 'appointments'),
