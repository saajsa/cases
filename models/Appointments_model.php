    ...
    public function get_appointments($filters = [])
{
    ...
    if ($this->db->table_exists(db_prefix() . 'appointment_services')) {
        $this->db->select('s.name as service_name, s.price as service_price, s.duration_minutes as service_duration');
        $this->db->join(db_prefix() . 'appointment_services s', 's.id = a.service_id', 'left');
    }

    if ($this->db->table_exists(db_prefix() . 'staff')) {
        $this->db->select('st.firstname as staff_firstname, st.lastname as staff_lastname');
    ...
    if ($this->db->table_exists(db_prefix() . 'contacts')) {
        $this->db->select('CONCAT(COALESCE(ct.firstname, ""), " ", COALESCE(ct.lastname, "")) as contact_name');
        $this->db->join(db_prefix() . 'contacts ct', 'ct.id = a.contact_id', 'left');
    }

    if ($this->db->table_exists(db_prefix() . 'invoices')) {
        $this->db->select('i.total as invoice_total, i.status as invoice_status');
